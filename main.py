...
@app.route('/')
def index():
    return render_template('index.html',symbol_list=symbol_list,dict_list=dict_list,n=n)
...
def get_niftybank_chart_data():    
    ...
    df_niftybank.drop(['Open','High','Low','Adj Close','Volume'],axis=1,inplace=True)
    df_niftybank["Date"] = pd.to_datetime(df_niftybank["Date"])
    df_niftybank.dropna(inplace=True)
    chart_data = [{"time": date.strftime("%Y-%m-%d"), "value": close} for date, close in zip(df_niftybank["Date"], df_niftybank["Close"])]
    return jsonify(chart_data)
...
def get_niftyfin_chart_data():    
    ...
    df_niftyfin.drop(['Open','High','Low','Adj Close','Volume'],axis=1,inplace=True)
    df_niftyfin["Date"] = pd.to_datetime(df_niftyfin["Date"])
    df_niftyfin.dropna(inplace=True)
    chart_data = [{"time": date.strftime("%Y-%m-%d"), "value": close} for date, close in zip(df_niftyfin["Date"], df_niftyfin["Close"])]
    return jsonify(chart_data)
...
def get_reliance_chart_data():    
    ...
    df_reliance.drop(['Open','High','Low','Adj Close','Volume'],axis=1,inplace=True)
    df_reliance["Date"] = pd.to_datetime(df_reliance["Date"])
    df_reliance.dropna(inplace=True)
    chart_data = [{"time": date.strftime("%Y-%m-%d"), "value": close} for date, close in zip(df_reliance["Date"], df_reliance["Close"])]
    return jsonify(chart_data)
